...
public class reindeer : MonoBehaviour {
	...
	float[] Turnabilitys    = {5.0f, 10.0f,  15.0f, 25.0f, 50.0f};
    ...
    public void Update()
    {
        enteredorstayedinanotherpieceofice = false;
    ...
    public void rotateTowards(Vector2 _target){
        ...
        if (onIce)
        {
            forward = Quaternion.Euler(0, 0, angle * turnability * 0.3f * Time.deltaTime) * forward;
        }
        else
        {
    ...
    bool enteredorstayedinanotherpieceofice = false;
    void OnTriggerEnter2D(Collider2D _col)
    {
        if (_col.name == "ice(Clone)")
        {
            onIce = true;
            enteredorstayedinanotherpieceofice = true;
        }
    }
    ...
    {
        if (_col.name == "ice(Clone)" && enteredorstayedinanotherpieceofice == false)
        { onIce = false; }
    }
}
